...
function showPage(id){document.querySelectorAll(".page").forEach(x=>x.classList.remove("active"));document.getElementById(id).classList.add("active");renderAll();populate();window.scrollTo(0,0)}
...
function renderAll(){
 ...
 document.getElementById("businessName").value=db.settings.name;
 document.getElementById("businessGstin").value=db.settings.gstin;
 document.getElementById("businessAddress").value=db.settings.address;
 document.getElementById("businessPhone").value=db.settings.phone;
}
...
function newInvoice(){showPage("billing");document.getElementById("billDate").value=today();document.getElementById("billPaid").value=0;document.getElementById("billRows").innerHTML="";addBillRow()}
function addBillRow(){
 const div=document.createElement("div");div.className="row";
 div.innerHTML=`<select class="bprod" onchange="calcBill()"><option value="">Product</option>${db.products.map((p,i)=>`<option value="${i}">${esc(p.name)}</option>`).join("")}</select><input class="bqty" type="number" min="1" value="1" oninput="calcBill()"><input class="brate" type="number" min="0" value="0" oninput="calcBill()"><input class="bgst" type="text" readonly placeholder="GST"><button class="small" onclick="this.parentElement.remove();calcBill()">×</button>`;
...
function calcBill(){
 ...
 document.getElementById("billSubtotal").textContent=money(sub);document.getElementById("billGst").textContent=money(gst);document.getElementById("billTotal").textContent=money(sub+gst);
 let paid=+document.getElementById("billPaid").value||0;document.getElementById("billBalance").textContent=money(Math.max(0,sub+gst-paid))
}
function saveInvoice(){
 let items=[];
 document.querySelectorAll("#billRows .row").forEach(r=>{let i=r.querySelector(".bprod").value,p=db.products[i];if(p){let q=+r.querySelector(".bqty").value||0,rate=+r.querySelector(".brate").value||0;items.push({productId:p.id,name:p.name,qty:q,rate,gst:p.gst})}});
 ...
 db.invoices.push(inv);save();renderAll();printInvoice(inv.id)
}
function printInvoice(id){
 ...
 let w=window.open("","_blank");if(!w){alert("Please allow pop-ups to print the invoice.");return}
 w.document.write(`<html><head><title>${x.number}</title><style>body{font-family:Arial;padding:25px;color:#111}h1{margin-bottom:2px}table{width:100%;border-collapse:collapse;margin-top:25px}th,td{border:1px solid #ccc;padding:8px;text-align:left}.right{text-align:right}hr{border:0;border-top:1px solid #ccc}</style></head><body><h1>${esc(db.settings.name)}</h1><div>${esc(db.settings.address)}<br>${esc(db.settings.phone)} ${db.settings.gstin?"| GSTIN: "+esc(db.settings.gstin):""}</div><hr><h2>TAX INVOICE</h2><b>Invoice:</b> ${x.number}<br><b>Date:</b> ${x.date}<br><b>Customer:</b> ${esc(x.customerName)}<table><tr><th>Product</th><th>Qty</th><th>Rate</th><th>GST</th><th>Total</th></tr>${rows}</table><h3 class="right">Subtotal: ${money(x.subtotal)}</h3><h3 class="right">GST: ${money(x.gst)}</h3><h2 class="right">Grand Total: ${money(x.total)}</h2><p class="right">Paid: ${money(x.paid)}<br>Balance: ${money(x.total-x.paid)}</p><p>Thank you for your business.</p><script>window.print()<\/script></body></html>`);w.document.close()
}
function openProductForm(idx=null){
 let p=idx==null?{name:"",hsn:"",gst:18,purchase:0,sale:0,stock:0}:db.products[idx];
 openModal(`<h2>${idx==null?"Add":"Edit"} Product</h2><label>Name</label><input id="fName" value="${esc(p.name)}"><label>HSN</label><input id="fHsn" value="${esc(p.hsn)}"><div class="formgrid"><div><label>GST %</label><input id="fGst" type="number" value="${p.gst}"></div><div><label>Stock</label><input id="fStock" type="number" value="${p.stock}"></div><div><label>Purchase Rate</label><input id="fPurchase" type="number" value="${p.purchase}"></div><div><label>Sale Rate</label><input id="fSale" type="number" value="${p.sale}"></div></div><button class="primary full" onclick="saveProduct(${idx})">Save Product</button>`)
}
function saveProduct(idx){let o={id:idx==null?Date.now().toString():db.products[idx].id,name:fName.value,hsn:fHsn.value,gst:+fGst.value||0,stock:+fStock.value||0,purchase:+fPurchase.value||0,sale:+fSale.value||0};if(!o.name)return alert("Product name required");if(idx==null)db.products.push(o);else db.products[idx]=o;save();closeModal();renderAll();populate()}
function editProduct(i){openProductForm(i)}
function openCustomerForm(idx=null){
 let c=idx==null?{name:"",phone:"",gstin:"",address:""}:db.customers[idx];
 openModal(`<h2>${idx==null?"Add":"Edit"} Customer</h2><label>Name</label><input id="cName" value="${esc(c.name)}"><label>Phone</label><input id="cPhone" value="${esc(c.phone)}"><label>GSTIN</label><input id="cGstin" value="${esc(c.gstin)}"><label>Address</label><textarea id="cAddress">${esc(c.address)}</textarea><button class="primary full" onclick="saveCustomer(${idx})">Save Customer</button>`)
}
function saveCustomer(idx){let o={id:idx==null?Date.now().toString():db.customers[idx].id,name:cName.value,phone:cPhone.value,gstin:cGstin.value,address:cAddress.value};if(!o.name)return alert("Customer name required");if(idx==null)db.customers.push(o);else db.customers[idx]=o;save();closeModal();renderAll();populate()}
function editCustomer(i){openCustomerForm(i)}
function savePurchase(){
 let i=+purchaseProduct.value,p=db.products[i];if(!p)return alert("Add a product first.");
 let q=+purchaseQty.value||0,r=+purchaseRate.value||0;
 if(q<=0)return alert("Enter a valid quantity.");
 p.stock=Number(p.stock)+q;db.purchases.push({date:purchaseDate.value||today(),supplier:purchaseSupplier.value||"Supplier",productName:p.name,qty:q,rate:r});save();renderAll();alert("Purchase saved and stock updated.")
}
...
function resetData(){if(confirm("Delete all local data? This cannot be undone.")){localStorage.removeItem(KEY);location.reload()}}
